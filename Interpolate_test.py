# -*- coding: utf-8 -*-
"""
Created on Mon May 13 13:23:08 2019
...
def consolidate(folder):
    ...
    #creates the overlay
    labels = ['23C', '100C', '150C', '160C', '180C', '200C', '210C']
...
#        plt.plot(strain[i][len(grd)], stress[i][len(grd)],'ro')
        ...
        #plt.figure(figsize = (20,10))
        plt.plot(strain[i][0:len(grd)], stress[i][0:len(grd)], label= labels[i])
...
#                plt.plot(grd3, 'go')
                ...
                SAB = strains[0:len(grd1)].max()
                SAB_list.append(SAB)
                StressAB = stresses[0:len(grd)].max()
...
#####################################################################################################################################################
    ...
    with pd.ExcelWriter(name) as writer: 
        ...
        #yp.to_excel(writer, sheet_name='yp')
